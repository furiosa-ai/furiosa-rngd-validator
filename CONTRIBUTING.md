...
1. **Script**: create `scripts/phases/run_<name>.sh`, using an existing phase (for example `scripts/phases/run_diag.sh`) as a template. The script should set `set -euo pipefail`, derive `SCRIPT_DIR` and `SCRIPTS_ROOT` the same way, source `$SCRIPTS_ROOT/lib/common.sh` for colors, logging, NPU detection, `apply_acs_mode`, `capture_dmesg`, and the shared benchmark/report helpers (`hal_bench`, `print_summary`, `use_furiosa_venv`, sensor monitor, ...), default its output directory to `$RUN_DIR/<name>`, write `PF_result.log` (plain text) and `PF_result.html` (HTML fragment, no `<html>/<head>/<body>` wrapper) via `lib/html.sh` helpers, and exit non-zero on failure. `generate_index.py` reads the fragment, embeds it inline in `index.html`, and then deletes the individual `PF_result.html` file.
2. **Registration**: add `<name>` to the phase list of the `for phase in ...` loop in `entrypoint.sh`, in run order. The loop runs `phases/run_<name>.sh` and records its exit code at `$RUN_DIR/<name>/exit_code.txt`.
